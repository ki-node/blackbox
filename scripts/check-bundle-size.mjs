...
for (const asset of assets) {
  const extension = Object.keys(budgets).find((candidate) =>
    asset.endsWith(candidate),
  );
  if (!extension) continue;

  const compressedSize = gzipSync(
    await readFile(join(assetDirectory.pathname, asset)),
  ).byteLength;
  const budget = budgets[extension];
  if (compressedSize > budget) {
    throw new Error(
      `${asset}: ${compressedSize} bytes gzip exceeds ${budget} byte budget`,
    );
  }
